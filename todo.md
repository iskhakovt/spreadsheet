...
- [ ] `p2` Caching audit — a few related wins worth batching:
    - **Stabilize localStorage-backed state** via `useSyncExternalStore`. `getAnswers()`, `getPendingOps()`, `getSelectedCategories()`, `getSelectedTier()`, etc. return fresh object identity each call. This defeats at least two `useMemo` blocks in the codebase (`Summary.tsx` `grouped` and `Review.tsx` `grouped` both list `answers` as a dep — memo re-runs every render). Fix: wrap each localStorage key in a per-key external store with cached snapshot + storage-event + custom same-tab event. Adds subscribe-on-write plumbing to every setter, but yields stable identity, working `useMemo`, and cross-tab reactivity for free. Also enables the planned follow-up on `Question.tsx`'s welcome-screen `hasAnswersInCategory` / `firstUnansweredInCategoryIdx` per-render scans.
    - **Memoize `sortMembersViewerFirst`** call in `PersonApp.tsx:150` — `useMemo` with `[status.members, status.person.id]`. Cheap (<8 members typically) but free now.
    - **Question-by-id lookup Map** parallel to the existing `categoryMap` — `useMemo(() => new Map(questions.map((q) => [q.id, q])))` in `Question.tsx` (or push into the `questions.list` queryFn for global reuse). Eliminates `questions.find((q) => q.id === ...)` linear scans (used in `Review.tsx` for `categories.find(...)` inside the loop over all questions — O(n·m) today).
