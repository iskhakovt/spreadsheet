...
- [ ] `p3` Dedupe pending-ops on note-debounce keystrokes — a 50-char note currently produces ~6 pending ops (one per debounce window). Correct via last-write-wins on replay, just wasteful. Either change pending-ops storage from `string[]` to `{key, op}[]` or add a key-aware `replacePendingOp(key, op)`.
...
- [ ] `p3` Memoize Comparison match rows — every WS journal push triggers a full `PairComparison` re-render and all ~20-50 match rows recompute. Extract row JSX into a memoed component. Profile first: only worth it if RUM or React Profiler shows lag during active sessions.
