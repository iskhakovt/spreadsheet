...
- [ ] `p2` Fix `mergeAfterRejection`'s server-side null-delete divergence (`packages/web/src/lib/journal.ts`). The function iterates *server state* (post-replay) and overlays the outbox, but a deleted key (replayed from a `data: null` op) is absent from server-state and the merge leaves prev's value alone. Result: full-replay (bootstrap, fresh device) sees the deletion correctly, but delta-replay (same-device resume, WS echo) preserves the stale answer. Today no UI affordance emits `data: null`, so this is dormant. Becomes a divergence bug if "clear my answer" ever ships. Proper fix: rewrite `mergeAfterRejection` to walk the server *operations* in order (skip keys with a pending op, otherwise apply set-or-delete directly to merged) instead of replaying-then-iterating. Add a test that exercises `data: null` through the delta path. ~30 LOC + test.
