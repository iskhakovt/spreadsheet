...
describe("SyncStore.push", () => {
  ...
  it("updates progress", async () => {
    ...
    expect(row.progress).toBe(plainProgress({ answered: 1, total: 10 }));
  });
  ...
  it("returns encryption_mismatch when any op in a batch mismatches", async () => {
    ...
    const result = await store.push(
      ...
      {
        stoken: null,
        operations: [plainOp("a:mutual", { rating: "yes", note: null }), "e:1:encryptedpayload"],
        progress: undefined,
      },
      false,
    );
...
describe("SyncStore.journalSince", () => {
  ...
  it("returns only entries with id > sinceId when sinceId is set", async () => {
    ...
    await store.push(
      ...
      {
        stoken: null,
        operations: [plainOp("a:receive", { rating: "maybe", note: null })],
        progress: undefined,
      },
      false,
    );
