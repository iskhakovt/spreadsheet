...
- **Question flow** — `Screen` discriminated union (`welcome` | `question`). Welcome interstitials at category boundaries. All categories on by default, managed from Summary screen. Timing sub-question ("now or later?") controlled by `group.showTiming`. Visibility per question is computed by `lib/visibility.ts`'s `visibleSides()` — combines anatomy targeting + tier filter + dependency gating (transitive, per-side for give/receive). A welcome screen is skipped when its category has zero visible questions for the user.
