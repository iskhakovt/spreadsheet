...
for (const encrypted of [false, true]) {
  test.describe(`two-player completion flow (${encrypted ? "encrypted" : "plaintext"})`, () => {
    test("Alice and Bob both complete and see results", async ({ browser }) => {
      ...
      await expect(bob.getByText("Your results")).toBeVisible({ timeout: 3000 });
      await expect(bob.getByText("Alice & Bob")).toBeVisible({ timeout: 3000 });
      ...
      await expect(alice.getByText("Alice & Bob")).toBeVisible({ timeout: 3000 });
