...
import { answerAllQuestions, goThroughIntro, setCategories } from "./helpers.js";
...
test.describe("filtered mode — self-pick anatomy flow", () => {
  test("both players pick anatomy, wait for each other, then answer", async ({ browser }) => {
    ...
    await expect(alice.getByText("You're all set")).toBeVisible({ timeout: 10000 });
    ...
    await expect(alice.getByText("One quick thing")).toBeVisible({ timeout: 5000 });
    ...
    await expect(alice.getByText("Almost there")).toBeVisible({ timeout: 10000 });
    ...
    await expect(bob.getByText("One quick thing")).toBeVisible({ timeout: 5000 });
    ...
    await expect(bob.getByText("Here's how it works")).toBeVisible({ timeout: 10000 });

    // Alice's 5s fast poll on /pending picks up that group is ready → guard redirects to /intro
    await expect(alice.getByText("Here's how it works")).toBeVisible({ timeout: 15000 });
