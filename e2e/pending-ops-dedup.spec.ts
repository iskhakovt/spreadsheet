...
test.describe("pending-op dedup", () => {
  test("debounced note keystrokes collapse to a single queued op", async ({ alice }) => {
    ...
    // Read local answer state for the question key. Plaintext in encrypted
    // mode too (encryption only wraps the wire op), so it's a reliable
